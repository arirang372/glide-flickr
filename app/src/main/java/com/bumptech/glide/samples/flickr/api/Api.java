...
//https://api.flickr.com/services/rest/?method=flickr.photos.getRecent&per_page=2000&format=json&api_key=f0e6fbb5fdf1f3842294a1d21f84e8a6
...
public final class Api {
  ...
  private static final LruCache<UrlCacheKey, String> CACHED_URLS = new LruCache<>(MAX_URLS_TO_CACHE);
  ...
  private static List<String> getLargerSizeKeys(int width, int height) {
    ...
    for (int i = 0; i < SORTED_SIZE_KEYS.size(); i++) {
  ...
  public void query(Query query) {
    if (lastQueryResult != null && lastQueryResult.getQuery().equals(query)) {
      for (QueryListener listener : queryListeners) {
        listener.onSearchCompleted(lastQueryResult.getQuery(), lastQueryResult.getResults());
      }
      return;
    }
